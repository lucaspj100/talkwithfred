      ...
      {loading && !metrics ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            <Link to="/admin/users" className="block rounded-2xl transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-ring">
              <AdminMetricCard label="Total de usuários" value={metrics.total_users} icon={Users} hint="Ver todos" />
            </Link>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            {(Object.keys(statusCounts) as EngagementStatus[]).map((k) => (
              <Link
                ...
                to="/admin/users"
                search={{ status: k }}
                className="block rounded-2xl transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-ring"
              >
                <AdminMetricCard label={ENGAGEMENT_LABELS[k]} value={statusCounts[k]} hint="Ver usuários" />
              </Link>
            ))}
          </div>
